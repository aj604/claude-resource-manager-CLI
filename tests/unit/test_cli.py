...
def mock_catalog_loader():
    ...
    with patch("claude_resource_manager.core.catalog_loader.CatalogLoader") as mock:
        ...
        instance.load_resource = AsyncMock(
            return_value=Mock(
                id="test-resource",
                type="agent",
                name="Test Resource",
...
class TestBrowseCommand:
    ...
    def test_WHEN_browse_command_THEN_launches_tui(self, cli_runner, mock_catalog_loader):
    ...
    def test_WHEN_browse_with_type_filter_THEN_filters_resources(self, cli_runner):
    ...
    def test_WHEN_browse_with_category_filter_THEN_filters_resources(self, cli_runner):
...
class TestInstallCommand:
    ...
    def test_WHEN_install_with_deps_THEN_installs_all(
        ...
        mock_resource = Resource(
            ...
            source={"url": "https://example.com/test.md", "repo": "test", "path": "test.md"},
            install_path="~/.claude/agents/architect.md",
            dependencies=Dependency(required=["dependency-1", "dependency-2"], recommended=[]),
        )
        mock_catalog_loader.load_resource = AsyncMock(return_value=mock_resource)
...
class TestDepsCommand:
    ...
    def test_WHEN_deps_command_THEN_shows_tree(self, cli_runner, mock_catalog_loader):
        ...
        mock_resource = Mock(
            id="test-resource", dependencies=Dependency(required=["dep-1", "dep-2"], recommended=[])
        )
        mock_catalog_loader.load_resource = AsyncMock(return_value=mock_resource)
    ...
    def test_WHEN_deps_reverse_flag_THEN_shows_dependents(self, cli_runner, mock_catalog_loader):
        ...
        mock_resource = Mock(id="test-resource", dependencies=None)
        mock_catalog_loader.load_resource = AsyncMock(return_value=mock_resource)
    ...
    def test_WHEN_sync_command_THEN_updates_catalog(self, cli_runner, tmp_path):
        ...
        with patch("claude_resource_manager.cli.__file__", fake_cli_file):
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.return_value = Mock(returncode=0, stdout="Synced successfully", stderr="")
        ...
        with patch("claude_resource_manager.cli.__file__", fake_cli_file):
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.return_value = Mock(
                    returncode=0, stdout="Force synced successfully", stderr=""
                )
...
class TestGlobalOptions:
    ...
    def test_WHEN_verbose_flag_THEN_shows_debug_output(
        self, cli_runner, mock_catalog_loader, mock_search_engine
    ):
    ...
    def test_WHEN_quiet_flag_THEN_minimal_output(
        self, cli_runner, mock_catalog_loader, mock_search_engine
    ):
    ...
    def test_WHEN_catalog_path_specified_THEN_uses_custom_path(
        self, cli_runner, mock_catalog_loader, mock_search_engine, tmp_path
    ):
        ...
        result = cli_runner.invoke(cli, ["--catalog-path", str(custom_path), "search", "test"])
...
class TestErrorHandling:
    ...
    def test_WHEN_catalog_not_found_THEN_helpful_error(self, cli_runner):
        ...
        with patch("claude_resource_manager.core.catalog_loader.CatalogLoader") as mock_loader:
            ...
            instance = Mock()
            instance.load_all_resources = AsyncMock(
    ...
    def test_WHEN_network_error_THEN_helpful_error(
        self, cli_runner, mock_installer, mock_catalog_loader
    ):
        ...
        mock_installer.install = AsyncMock(side_effect=httpx.ConnectError("Network unreachable"))
    ...
    def test_WHEN_permission_error_THEN_helpful_error(
        self, cli_runner, mock_installer, mock_catalog_loader
    ):
        ...
        mock_installer.install = AsyncMock(side_effect=PermissionError("Permission denied"))
